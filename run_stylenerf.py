    ...
    while True:
        for batch_data in tqdm(train_dataloader):
            ...
            if global_step % args.i_weights == 0 and global_step > 0 or global_step >= args.origin_step:
                ...
                # Delete ckpts
                ckpts = [ckpt_dir_nerf / f for f in sorted(ckpt_dir_nerf.glob('*')) if 'tar' in str(f)]
...
def train_style_nerf(args, global_step, samp_func, samp_func_fine, nerf, nerf_fine, sv_path, nerf_gen_data_path):
    ...
    ckpts_style = [ckpt_dir_style / f for f in sorted(ckpt_dir_style.glob('*')) if 'tar' in str(f) and 'style' in str(f) and 'latent' not in str(f)]
    ...
    latent_ckpts = [ckpt_dir_latent / f for f in sorted(ckpt_dir_latent.glob('*')) if 'tar' in str(f) and 'style' not in str(f) and 'latent' in str(f)]
    ...
    ckpts = [ckpt_dir_decoder / f for f in sorted(ckpt_dir_decoder.glob('*')) if 'decoder_iter_' in str(f)]
    ...
    while True:
        for _ in range(rounds_per_epoch):
            ...
            if global_step % args.i_weights == 0 and global_step > 0:
                ...
                print('Saved checkpoints at', path)
                # Delete ckpts
                ckpts = [ckpt_dir_style / f for f in sorted(ckpt_dir_style.glob('*')) if 'tar' in str(f) and 'style' in str(f) and 'latent' not in str(f)]
                if len(ckpts) > args.ckp_num:
                    os.remove(ckpts[0])
...
def run(args):
    ...
    ckpts = [ckpt_dir_nerf / f for f in sorted(ckpt_dir_nerf.glob('*')) if 'tar' in str(f) and 'style' not in str(f) and 'latent' not in str(f)]
