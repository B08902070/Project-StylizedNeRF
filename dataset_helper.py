...
def style_data_prepare(style_path, content_images, size=512, chunk=64, sv_path=None, decoder_dir='./pretrained/decoder/', save_geo=True, no_reload=False):
    ...
    for i in tqdm(range(len(images_path))):
        ...
        print('style_imgs', style_img)
        print('content img', content_images[0])
        start = 0
        while start < content_images.shape[0]:
            end = min(start + chunk, content_images.shape[0])
            tmp_imgs = np.array([img_trans(img) for img in content_images[start:end]])
            tmp_imgs = torch.movedim(torch.from_numpy(tmp_imgs).float().to(device), -1, 1)
            with torch.no_grad():
